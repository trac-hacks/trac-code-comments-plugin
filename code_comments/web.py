...
class JSDataForRequests(CodeComments):
    ...
    def changeset_js_data(self, req, data):
        return {'repository': data['reponame'], 'page': 'changeset', 'revision': data['new_rev'], 'path': '', 'selectorToInsertBefore': 'div.diff:first'}

    def browser_js_data(self, req, data):
        return {'repository': data['reponame'], 'page': 'browser', 'revision': data['rev'], 'path': data['path'], 'selectorToInsertBefore': 'table#info'}
